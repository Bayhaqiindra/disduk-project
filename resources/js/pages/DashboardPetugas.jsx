...
const DashboardPetugas = () => {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      ...
      dikembalikan: 'bg-red-50 text-red-600 border-red-200',
    ...
    <Layout>
        ...
        <div className="space-y-8">
          ...
          {loadingStats ? (
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="bg-gray-50 border border-gray-100 h-24 rounded-2xl animate-pulse"></div>
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="bg-white border border-gray-200 shadow-sm p-5 rounded-2xl">
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Total Pengajuan</p>
                <h3 className="text-3xl font-black text-gray-900 mt-1.5">{stats.total}</h3>
              </div>
              <div className="bg-white border border-gray-200 shadow-sm p-5 rounded-2xl">
                <p className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">Menunggu</p>
                <h3 className="text-3xl font-black text-amber-500 mt-1.5">{stats.menunggu}</h3>
              </div>
              <div className="bg-white border border-gray-200 shadow-sm p-5 rounded-2xl">
                <p className="text-[10px] font-bold text-orange-600 uppercase tracking-widest">Diproses</p>
                <h3 className="text-3xl font-black text-orange-500 mt-1.5">{stats.diproses}</h3>
              </div>
              <div className="bg-white border border-gray-200 shadow-sm p-5 rounded-2xl">
                <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Selesai</p>
                <h3 className="text-3xl font-black text-emerald-500 mt-1.5">{stats.selesai}</h3>
              </div>
              <div className="bg-white border border-gray-200 shadow-sm p-5 rounded-2xl">
                <p className="text-[10px] font-bold text-rose-600 uppercase tracking-widest">Dikembalikan</p>
                <h3 className="text-3xl font-black text-rose-500 mt-1.5">{stats.dikembalikan}</h3>
              </div>
            </div>
          )}
          ...
          <div className="bg-white border border-gray-200 shadow-sm rounded-3xl p-6">
            <h3 className="text-base font-bold text-gray-900 mb-6">Aktivitas Pengajuan Terbaru</h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 text-xs">
                  {recentList.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="py-8 text-center text-gray-500">Belum ada pengajuan.</td>
                    </tr>
                  ) : (
                    recentList.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        <td className="py-3.5 font-bold text-gray-800">{item.nomor_pengajuan}</td>
                        <td className="py-3.5 text-gray-600 font-semibold">{item.layanan?.nama}</td>
                        <td className="py-3.5 text-gray-500">{new Date(item.created_at).toLocaleDateString('id-ID')}</td>
                        <td className="py-3.5">
                        ...
                        <td className="py-3.5 text-right">
                          <button 
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-[10px] font-bold rounded-lg text-gray-700 transition"
                          >
      ...
      {activeTab === 'ajukan' && (
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between border-b border-gray-200 pb-5 mb-8">
            <h2 className="text-xl font-black text-gray-900">Ajukan Berkas Baru</h2>
            <div className="flex items-center gap-2">
              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border transition ${
                submitStep >= 1 ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-50 border-gray-200 text-gray-400'
              }`}>1</span>
              <div className="w-8 h-0.5 bg-gray-200"></div>
              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border transition ${
                submitStep >= 2 ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-50 border-gray-200 text-gray-400'
              }`}>2</span>
            </div>
          </div>
          ...
          {submitStep === 1 && (
            <div className="space-y-6">
              <p className="text-sm text-gray-500">Pilih jenis layanan administrasi yang ingin Anda ajukan di bawah ini:</p>
              ...
              {loadingCategories ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {categories.map((svc) => {
                    ...
                    const color = colorMap[svc.kode] || 'from-gray-500 to-gray-600';
                    return (
                      <div 
                        key={svc.id}
                        onClick={() => {
                        ...
                        className="relative group p-6 bg-white border border-gray-200 hover:border-blue-400 hover:shadow-md rounded-2xl transition cursor-pointer overflow-hidden shadow-sm"
                      >
                        ...
                        <h3 className="text-sm font-bold text-gray-900 mb-1.5 group-hover:text-blue-600 transition">{svc.nama}</h3>
                        <p className="text-[11px] text-gray-500 leading-normal mb-3">{svc.deskripsi || 'Pengurusan dokumen kependudukan'}</p>
                        <div className="inline-flex items-center gap-1 text-[10px] font-black text-blue-600">
          ...
          {submitStep === 2 && selectedCategory && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <button 
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-gray-500 hover:text-gray-900 transition mb-4"
              >
                <ArrowLeft className="w-4 h-4" /> Kembali ke Kategori
              </button>
              <div className="p-6 bg-white border border-blue-200 rounded-2xl shadow-sm">
                <h3 className="text-lg font-bold text-gray-900 mb-2">Pilih Jenis Layanan {selectedCategory.nama}</h3>
                <p className="text-sm text-gray-500 mb-6">Pilih salah satu spesifikasi layanan yang sesuai dengan kebutuhan pemohon:</p>
                <div className="space-y-3">
                  {selectedCategory.sub_layanan.map(sub => (
                    <div 
                      ...
                      className="p-4 bg-gray-50 border border-gray-200 hover:border-blue-500 hover:bg-blue-50 rounded-xl cursor-pointer transition flex items-center justify-between group"
                    >
                      <div>
                        <h4 className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition">{sub.nama}</h4>
                        {sub.deskripsi && <p className="text-xs text-gray-500 mt-1">{sub.deskripsi}</p>}
                      </div>
          ...
          {submitStep === 3 && (selectedCategory || selectedSubService) && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-6 h-fit shadow-sm">
                <button 
                  onClick={() => {
                ...
                <div className="border-b border-gray-200 pb-4">
                  <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Layanan Terpilih</span>
                  <h3 className="text-base font-extrabold text-gray-900 mt-1">{selectedSubService ? selectedSubService.nama : selectedCategory.nama}</h3>
                </div>

                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-gray-900">1. Unduh Template Formulir</h4>
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-600 font-bold rounded-xl text-xs transition"
                  >
                ...
                <div className="space-y-3 pt-4 border-t border-gray-200">
                  <h4 className="text-xs font-bold text-gray-900">Persyaratan Dokumen:</h4>
                  {loadingPersyaratan ? (
                    <div className="flex justify-center p-4"><Loader2 className="w-4 h-4 animate-spin text-blue-600" /></div>
                  ) : (
                    <ul className="space-y-2">
                      {persyaratanList.map((r) => (
                        <li key={r.id} className="flex items-start gap-2 text-[11px] text-gray-600">
                          {r.is_wajib ? (
                            <Check className="w-4 h-4 text-green-500 shrink-0 mt-0.5" />
                          ) : (
                            <div className="w-4 h-4 border border-gray-300 rounded-sm shrink-0 mt-0.5" />
                          )}
                          <span>
                            <span className={r.is_wajib ? "font-bold text-gray-900" : ""}>{r.nama_dokumen}</span>
                            {!r.is_wajib && <span className="text-blue-600 ml-1">(Opsional)</span>}
                            {r.catatan && <span className="block text-[9px] text-gray-500 italic mt-0.5">{r.catatan}</span>}
                          </span>
                        </li>
                      ))}
                    </ul>
              ...
              <div className="lg:col-span-2 bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
                <h3 className="text-base font-bold text-gray-900 mb-6">Upload Berkas Pemohon</h3>
                ...
                <form onSubmit={handleSubmitPengajuan} className="space-y-6">
                  ...
                  <div className="p-5 bg-gray-50 border border-gray-200 rounded-2xl">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h4 className="text-xs font-bold text-gray-900">Upload Formulir Pendaftaran <span className="text-red-500">*</span></h4>
                        <p className="text-[10px] text-gray-500 mt-0.5">Wajib formulir cetak yang diisi & ditandatangani.</p>
                      </div>
                      {formulirFile && <span className="px-2 py-0.5 bg-green-50 text-green-600 border border-green-200 text-[9px] font-bold rounded-md uppercase">Terpilih</span>}
                    </div>
                    
                    <label className="flex items-center gap-3 px-4 py-3 bg-white border border-gray-300 hover:border-gray-400 rounded-xl cursor-pointer transition text-xs text-gray-600 hover:text-gray-900">
                  ...
                  {/* Requirements upload */}
                  <div className="space-y-4">
                    <h4 className="text-xs font-bold text-gray-900">Upload Berkas Persyaratan Tambahan</h4>
                    
                    {loadingPersyaratan ? (
                      <div className="p-8 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-gray-500" /></div>
                    ) : (
                      persyaratanList.map((r) => (
                        <div key={r.id} className={`p-4 bg-gray-50 border border-gray-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${!r.is_wajib && !reqFiles[r.id] ? 'opacity-80' : ''}`}>
                          <div className="max-w-md">
                            <p className="text-xs font-bold text-gray-900 leading-snug">
                              {r.nama_dokumen}
                              {r.is_wajib && <span className="text-red-500 ml-1">*</span>}
                              {!r.is_wajib && <span className="text-blue-600 ml-2 font-normal text-[10px] border border-blue-200 bg-white px-1.5 py-0.5 rounded">Opsional</span>}
                            </p>
                          ...
                          <label className={`flex items-center gap-2 px-4 py-2 bg-white hover:bg-gray-50 border rounded-xl cursor-pointer text-[11px] font-bold transition w-fit shrink-0 ${reqFiles[r.id] ? 'border-green-500 text-green-600 bg-green-50' : 'border-gray-300 hover:border-gray-400 text-gray-600 hover:text-gray-900'}`}>
                            {reqFiles[r.id] ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Upload className="w-3.5 h-3.5 text-gray-400" />}
                  ...
                  <button 
                    ...
                    className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md transition flex items-center justify-center gap-2 mt-4 disabled:opacity-50 disabled:pointer-events-none"
                  >
                    {submitting ? (
                      <>
      ...
      {activeTab === 'list' && (
        <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-6 shadow-sm">
          <h2 className="text-base font-bold text-gray-900">Daftar Semua Pengajuan Kependudukan Saya</h2>
                ...
                {loadingList ? (
                  [...Array(5)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td colSpan="5" className="py-4 bg-gray-50 h-10"></td>
                    </tr>
                  ))
                ) : submissions.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="py-12 text-center text-gray-500">Anda belum pernah mengirim pengajuan kependudukan.</td>
                  </tr>
                ) : (
                  submissions.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="py-4 font-bold text-gray-800">{item.nomor_pengajuan}</td>
                      <td className="py-4 text-gray-600 font-semibold">{item.layanan?.nama}</td>
                      <td className="py-4 text-gray-500">{new Date(item.created_at).toLocaleDateString('id-ID')}</td>
                      <td className="py-4">
                        <span className={`px-2.5 py-1 text-[9px] font-bold rounded-md border uppercase ${getStatusBadge(item.status)}`}>
                          {item.status}
                        </span>
          ...
          {pagination.last_page > 1 && (
            <div className="flex justify-center gap-2 pt-4 border-t border-gray-100">
              <button 
                ...
                className="px-3 py-1.5 rounded-lg bg-white border border-gray-200 text-[11px] font-bold hover:bg-gray-50 disabled:opacity-30 disabled:pointer-events-none"
              >
                Sebelumnya
              </button>
      ...
      {selectedSubmission && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-3xl bg-white border border-gray-200 rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col relative animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div>
                ...
                <h3 className="text-base font-extrabold text-gray-900 mt-2">{selectedSubmission.nomor_pengajuan}</h3>
              </div>
              <button 
                onClick={() => setSelectedSubmission(null)}
                className="p-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-400 hover:text-gray-900 rounded-xl transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-gray-50 border border-gray-200 rounded-2xl space-y-2">
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Jenis Layanan</p>
                  <p className="text-sm font-bold text-gray-900">{selectedSubmission.layanan?.nama}</p>
                </div>
                <div className="p-4 bg-gray-50 border border-gray-200 rounded-2xl space-y-2">
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Desa Pemohon</p>
                  <p className="text-sm font-bold text-gray-900">Desa {selectedSubmission.user?.desa}</p>
                </div>
              </div>
              ...
              {selectedSubmission.catatan_admin && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-2xl space-y-2">
                  <h4 className="text-xs font-bold text-red-600">Catatan Tanggapan Instansi:</h4>
                  <p className="text-xs text-red-700 leading-relaxed">{selectedSubmission.catatan_admin}</p>
                </div>
              )}

              {/* Uploaded Documents */}
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-gray-900">File & Dokumen Pengajuan</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {selectedSubmission.uploads?.map((up) => (
                    <div key={up.id} className="p-4 bg-white border border-gray-200 rounded-2xl flex items-center justify-between gap-4 shadow-sm">
                      <div className="overflow-hidden">
                        <p className="text-xs font-bold text-gray-800 truncate">{up.nama_file}</p>
                        <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mt-1">{up.tipe}</p>
                      </div>
                      ...
                      <div className="flex gap-2 shrink-0">
                        <a 
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="p-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-lg text-gray-600 hover:text-gray-900 transition"
                        >
                          <Eye className="w-4 h-4" />
                        </a>
                        <a 
                          href={`/api/dokumen/${up.id}/download`}
                          className="p-2 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg text-blue-600 transition"
                        >
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                <h4 className="text-xs font-bold text-gray-900">Histori Perubahan Status</h4>
                <div className="space-y-3">
                    ...
                    <div key={log.id} className="flex gap-4 items-start pl-2">
                      ...
                      <div>
                        <p className="text-xs font-bold text-gray-900">
                          Status diubah dari <span className="text-gray-500 font-semibold">{log.status_lama}</span> menjadi <span className="text-blue-600 font-extrabold uppercase">{log.status_baru}</span>
                        </p>
                        {log.catatan && <p className="text-[10px] text-gray-500 leading-normal mt-0.5">Catatan: {log.catatan}</p>}
